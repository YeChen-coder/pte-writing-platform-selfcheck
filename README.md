...
（3）自己放上去的例文随时可以修改，点“编辑例文” - 直接把新例文粘过去保存，就可以开始练习了 （在 deploy.md 这个文件里面，确实写了可以通过整体导入导出来处理这些题目的 JSON 文件。但是作者自己实际使用下来，觉得还是在页面上直接改更方便一点，不至于再去折腾 JSON 了。你就在页面直接改吧，多方便、多直接啊！大家都是来备考的，就别给自己在这方面添堵了。）
